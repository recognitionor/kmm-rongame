...
class CardDataSourceImpl(db: AppDatabase, private val httpClient: HttpClient) : CardDataSource {
    ...
    override fun getCardInfoList(): Flow<Resource<List<Card>>> = flow {
            ...
            supervisorScope {
                try {
                    ...
                    list.await().forEach {
                        ...
                        val test = ImageStorage.saveImage(imgArray)
    ...
    override fun getCardCombineInfoList(): Flow<Resource<List<CardType>>> = flow {
        emit(Resource.Loading())
                ...
                try {
                    ...
                    list.forEach {
                        queries.insertCardCombineEntity(
                            it.id.toLong(), it.item1.toLong(), it.item2.toLong(), it.result
                        )
                    }
                } catch (e: Exception) {
                    emit(Resource.Error(e.message.toString()))
